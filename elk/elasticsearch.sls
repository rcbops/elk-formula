{% with repo_key_file = '/root/elastic_repo.key' %}
{% with elastic_repo_loc = 'http://packages.elasticsearch.org/elasticsearch/1.3/debian' %}

elastic_repos_key:
  file.managed:
    - name: {{ repo_key_file }}
    - contents: |+
          -----BEGIN PGP PUBLIC KEY BLOCK-----
          Version: GnuPG v2.0.14 (GNU/Linux)

          
          =92oX
          -----END PGP PUBLIC KEY BLOCK-----
  cmd.run:
    - name: cat {{ repo_key_file }} | apt-key add -
    - require:
      - file: elastic_repos_key

elasticsearch_repo:
  file.managed:
    - name: /etc/apt/sources.list.d/elasticsearch.list
    - require:
      - cmd: elastic_repos_key
    - contents: deb {{ elastic_repo_loc }} stable main

{% endwith %}
{% endwith %}

openjdk-7-jre:
  pkg.installed

elasticsearch_soft:
  pkg.installed:
    - name: elasticsearch
    - require:
      - file: elasticsearch_repo
      - pkg: openjdk-7-jre 

/etc/elasticsearch/elasticsearch.yml:
  file:
    - managed
    - user: root
    - group: root
    - mode: 644
    - template: jinja
    - source: salt://elk-formula/files/elasticsearch/elasticsearch.yml

elastic_service:
  pkg.installed:
    - name: elasticsearch
    - require:
      - file: /etc/elasticsearch/elasticsearch.yml
  service.running:
    - name: elasticsearch
    - enable: True
    - watch:
      - file: /etc/elasticsearch/elasticsearch.yml
      - file: /etc/elasticsearch/config/realm.properties
    - require:
      - pkg: elasticsearch


install_jetty_plugin:
  cmd.run:
    - name: /usr/share/elasticsearch/bin/plugin -url https://oss-es-plugins.s3.amazonaws.com/elasticsearch-jetty/elasticsearch-jetty-1.2.1.zip -install elasticsearch-jetty-1.2.1

/etc/elasticsearch/config/realm.properties:
  file:
    - managed
    - user: root
    - group: root
    - mode: 644
    - template: jinja
    - makedirs: true
    - source: salt://elk-formula/files/elasticsearch/realm.properties

  